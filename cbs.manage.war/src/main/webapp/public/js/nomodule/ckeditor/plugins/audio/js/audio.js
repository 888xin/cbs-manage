		...
		moveStates : function(to_form, field) {
			...
			if (to_form) {
				if (data.type == 'video') {
					...
					//if audio is mp3,update address change to flashvars' soundFile
					if(data.type == 'audio'){
						//get the mp3 address
						var mp3Url = data.params.flashvars;
						var ms = "soundFile=";
						var inx = mp3Url.indexOf(ms);
						mp3Url = (inx != -1)? $.trim( mp3Url.substring(inx+ms.length)):0;
						//insert mp3
						if(inx != -1 && mp3Url.length > 0){							
							setVal('src', mp3Url);
						}
						//search mp3						
						else
