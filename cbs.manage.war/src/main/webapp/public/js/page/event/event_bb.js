define(function(require, exports, module) {
    ...
    $('#date_picker_bb').datepicker('option', {dateFormat: "yy-mm-dd" });
    //页面载入后显示当日日期
    ...
    function getAsyData(timeStamp, day) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: appName + "/event/basketball/getAsyData",
            data: {timeStamp: page_date.getTime(), fullFlag: full_flag, isFirst:is_first},
            success: function (data) {
                if (data.code == 200) {
                    //清空原有的数据
                    $("#bb-table tbody tr").remove();
                    if (data.number > 0) {
                        $.each(data.contests, function (index, contest) {
                            var htmlStr = showBbData(contest.contest_id, contest.color, contest.cup_name,contest.cup_id, contest.h_t.name,contest.h_t.t_id, contest.a_t.name,contest.a_t.t_id,
                            ...
                            $("#bb-table tbody").append(htmlStr);
                        });
                        if (contest_num == 0) {
                            $.gritter.add({
                                title: '<h3>该天无重要赛事！</h3>',
                                text: "<h4>这天无重要赛事，请尝试选择右上角的全部赛事选项，或点击此<a id='bb_a_full" + page_date.getTime() + "' href='#' class='blue'>链接</a></h4>",
                                image: appName + '/public/images/basketball.jpg',
                                sticky: false,
                                time: 2000,
                                class_name: 'gritter-success gritter-light'
                            });
                            $("#bb_a_full" + page_date.getTime()).on('click', function (e) {
                                e.preventDefault();
                                $("#yesterday_data").removeClass("disabled");
                                $("#tomorrow_date").removeClass("disabled");
                                $("#date_picker_bb").attr("disabled", false);
                                $("#more_data_p").addClass("hidden");
                                $("#more_data_search").addClass("hidden");
                                full_flag = true;
                                getAsyData(page_date.getTime(), 0);
                                $('#bb_dropdown_type').html('全部赛事');
                                $('#bb_table_head_title').html('全部赛事列表');
                            });
                        }
                    } else {
    ...
    function showBbData(contest_id, color, cup_name, cup_id, h_name,h_id, a_name,a_id, start_time, home_scores, away_scores, status,
                        bet_count, settle, odds_type, lock_flag, is_abnormal, level, init_count, home_count, away_count) {
        //统计
        contest_num++;
        var name = "";
        if (color != null) {
    ...
    //更新赛事
    var modal_bb_event_status = "";
    var modal_bb_event_level = "";
    $('#bb-table tbody').on('click', 'button.btn-info', function () {
        var tmpTr = $(this).closest("tr");
        const_contest_id = tmpTr.find("td:eq(0)").html();
        var modal_bb_cup_name = tmpTr.find("td:eq(1) span").html();
        var modal_bb_event_team = tmpTr.find("td:eq(2)").html();
        var modal_bb_event_time = tmpTr.find("td:eq(3)").html();
        //已结算 未结算
        var modal_bb_event_settle = tmpTr.find("td:eq(8) i").attr("title");
        //初始下注数
        //var modal_bb_event_initCount = Number(tmpTr.find("td:eq(7)").html());
        var modal_bb_event_homeCount = tmpTr.find("td:eq(12)").html();
        var modal_bb_event_awayCount = tmpTr.find("td:eq(13)").html();
        var modal_bb_event_scores = tmpTr.find("td:eq(4)").html();
        var scores = modal_bb_event_scores.split(":");
        var h_scores = parseInt(scores[0].trim());
        var a_scores = parseInt(scores[1].trim());
        modal_bb_event_status = tmpTr.find("td:eq(5) b").html();
        var modal_bb_event_lock = tmpTr.find("td:eq(9) i").attr("title");
        modal_bb_event_level = tmpTr.find("td:eq(10)").html();
        //锁定状态只显示文本值
        var appendStr = "";
        var is_lock = false;
        if (modal_bb_event_lock != "未锁定") {
            //锁定状态不能提交编辑
            //$("#edit_submit_button").addClass("disabled");
            is_lock = true;
        }
        appendStr = modalEditShowData(modal_bb_cup_name, modal_bb_event_team, modal_bb_event_time, modal_bb_event_settle,
            is_lock);
        ...
        $("#modal_bb_edit").modal({backdrop: 'static'});
            ...
            function (result) {
                if (result) {
                    ...
                    $.ajax({
                        type: 'POST',
                        dataType: 'json',
                        url: appName+"/event/basketball/settle",
    ...
    $("#modal_bb_edit").on("shown.bs.modal", function () {
        var status_value = "0";
        if (modal_bb_event_status == "完场") {
            status_value = "-1";
        } else if (modal_bb_event_status == "第一节") {
            status_value = "1";
        } else if (modal_bb_event_status == "第二节") {
            status_value = "2";
        } else if (modal_bb_event_status == "第二节") {
            status_value = "3";
        } else if (modal_bb_event_status == "第二节") {
            status_value = "4";
        } else if (modal_bb_event_status == "加时1") {
            status_value = "5";
        } else if (modal_bb_event_status == "加时2") {
            status_value = "6";
        } else if (modal_bb_event_status == "加时3") {
            status_value = "7";
        } else if (modal_bb_event_status == "中场") {
            status_value = "50";
        } else if (modal_bb_event_status == "待定") {
            status_value = "-2";
        } else if (modal_bb_event_status == "中断") {
            status_value = "-3";
        } else if (modal_bb_event_status == "取消") {
            status_value = "-4";
        } else if (modal_bb_event_status == "推迟") {
            status_value = "-5";
        }
        $('#modal_bb_select_status').find("option[value=" + status_value + "]").attr("selected", true);
        $('#modal_bb_select_level').find("option[value=" + modal_bb_event_level + "]").attr("selected", true);
    });
    //编辑窗口关闭时，去掉内容
    $('#modal_bb_edit').on('hidden.bs.modal', function () {
    ...
    $("#edit_submit_button").on("click", function () {
        ...
        var lock = $("#modal_bb_lock").prop('checked');
        var now_status = $('#modal_bb_select_status').val();
        var now_status_html = $('#modal_bb_select_status').find("option:selected").text();
        var now_level = $('#modal_bb_select_level').val();
        var cup_name = $('#modal_bb_edit_cup_name').val();
        var home_count = $('#home_count_spinner').val();
        var away_count = $('#away_count_spinner').val();
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: appName + "/event/basketball/update",
    ...
    $('#bb-table tbody').on('click', 'button.btn-warning', function () {
        var tmpTr = $(this).closest("tr");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 200) {
                    ...
                    var team_scores = "--:--";
                    var bb_status = oddsInfo.status;
                    if (bb_status != 0) {
                        team_scores = oddsInfo.contest.home_scores + ":" + oddsInfo.contest.away_scores;
                    }
//                    if (bb_status != 0 && bb_status != -2 && bb_status != -4 && bb_status != -5) {
//                        //设置赔率编辑里面的按钮不可用
...
//                    }
                    ...
                    //modal窗口打开
                    $("#modal_bb_odds").modal({backdrop: 'static'});
                    //胜平负 按钮样式变化
                    if (oddsInfo.op != null) {
    ...
    $("#odds_modal_op_edit_bt").on("click", function () {
        ...
        $.ajax({
            ...
            data: {contest_id: constant_contest_id, odds_id: constant_op_odds_id, home_roi: home, away_roi: away},
            success: function (data) {
                if (data.code == 200) {
                    $.gritter.add({
                        title: "赔率更改成功",
    ...
    $("#odds_modal_jc_edit_bt").on("click", function () {
        ...
        $.ajax({
            ...
            data: {contest_id: constant_contest_id, odds_id: constant_jc_odds_id, home_roi: home, away_roi: away, handicap: handicap},
            success: function (data) {
                if (data.code == 200) {
                    $.gritter.add({
                        title: "赔率更改成功",
    ...
    $("#odds_modal_size_edit_bt").on("click",function(){
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: appName+"/event/basketball/odds/size/update",
    ...
    $("#odds_modal_op_history_bt").on('click', function () {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: appName+"/event/basketball/odds/history",
            data: {contest_id:contest_id,odds_type:6},
            success: function (data) {
                if (data.code == 200) {
                    ...
                    var yData=[{name:'主胜',data:home_roi},{name:'客胜',data:away_roi}];
                    var chart = {
                           plotOptions: { 
                                series: { 
                                    cursor: 'pointer', 
                    ...
                    $("#modal_bb_odds_history").modal({backdrop: 'static'});
                } else {
                    $.gritter.add({
                        title: "出现" + data.code + "异常",
                        time: 2000,
    ...
    $("#odds_modal_jc_history_bt").on('click', function () {
        ...
        $.ajax({
            ...
            dataType: 'json',
            url: appName+"/event/basketball/odds/history",
            data: {contest_id:contest_id,odds_type:7},
            success: function (data) {
                if (data.code == 200) {
                    $("#modal_odds_history tbody tr").remove();
                    var xData=[];
                    ...
                    var yData=[{name:'让球',data:handicap},{name:'主胜',data:home_roi},{name:'客胜',data:away_roi}];
                    var chart = {
                           plotOptions: { 
                                series: { 
                                    cursor: 'pointer', 
    ...
    //删除赛事
    $('#bb-table tbody').on('click', 'button.btn-danger', function () {
        var tmpTr = $(this).closest("tr");
        var modal_bb_contest_id = tmpTr.find("td:eq(0)").html();
    ...
    //赛事类型切换
    $('#bb_dropdown_abnormal').on('click', function (e) {
        e.preventDefault();
        is_first = false ;
        $("#yesterday_data").addClass("disabled");
        $("#tomorrow_date").addClass("disabled");
        $("#date_picker_bb").attr("disabled", true);
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: appName + "/event/basketball/getAbnormalData",
            success: function (data) {
                if (data.code == 200) {
                    if (data.number > 0) {
                        $.each(data.contests, function (index, contest) {
                            ...
                            var htmlStr = showBbData(contest.contest_id, contest.color, contest.cup_name,contest.cup_id, contest.h_t.name, contest.a_t.name,
                                contest.start_time, contest.home_scores, contest.away_scores, contest.status, contest.bet_count, contest.settle,
        ...
        $('#bb_dropdown_type').html('异常赛事');
        $('#bb_table_head_title').html('异常赛事列表');
        $("#toolbar_info").attr("hidden", true);
    });
    $('#bb_dropdown_important').on('click', function (e) {
        e.preventDefault();
        is_first = false ;
        $("#yesterday_data").removeClass("disabled");
        $("#tomorrow_date").removeClass("disabled");
        ...
        $("#more_data_p").addClass("hidden");
        $("#more_data_search").addClass("hidden");
        $('#bb_dropdown_type').html('重要赛事');
        $('#bb_table_head_title').html('重要赛事列表');
        full_flag = false;
        contest_num = 0;
        $("#toolbar_info").attr("hidden", false);
        getAsyData(page_date.getTime(), 0);
    });
    $('#bb_dropdown_no_plate').on('click', function (e) {
        e.preventDefault();
        $('#bb_dropdown_type').html('未封盘');
    });
    $('#bb_dropdown_unlock').on('click', function (e) {
        e.preventDefault();
        $('#bb_dropdown_type').html('未锁定');
    });
    $('#bb_dropdown_no_settle').on('click', function (e) {
        e.preventDefault();
        $('#bb_dropdown_type').html('未结算');
    });
    $('#bb_dropdown_no_start').on('click', function (e) {
        e.preventDefault();
        $('#bb_dropdown_type').html('未开始');
    });
    $('#bb_dropdown_first').on('click', function (e) {
        e.preventDefault();
        is_first = true ;
        $("#yesterday_data").removeClass("disabled");
        $("#tomorrow_date").removeClass("disabled");
        ...
        $("#toolbar_info").attr("hidden", false);
        getAsyData(page_date.getTime(), 0);
        $('#bb_dropdown_type').html('一级赛事');
        $('#bb_table_head_title').html('一级赛事列表');
    });
    $('#bb_dropdown_all').on('click', function (e) {
        e.preventDefault();
        is_first = false ;
        $("#yesterday_data").removeClass("disabled");
        $("#tomorrow_date").removeClass("disabled");
    ...
    function modalEditShowData(cup_name, event_team, event_time, event_settle, is_lock) {
        if (is_lock) {
            is_lock = "<input id=\"modal_bb_lock\" type=\"checkbox\" checked class=\"ace ace-switch ace-switch-3\" />";
        } else {
            is_lock = "<input id=\"modal_bb_lock\" type=\"checkbox\" class=\"ace ace-switch ace-switch-3\" />";
        }
        var htmlStr = "<div class=\"profile-user-info profile-user-info-striped\">" +
            "                                        <div class=\"profile-info-row\">" +
            "                                            <div class=\"profile-info-name\"> 类型： </div>" +
            "                                            <div class=\"profile-info-value\">" +
            "                                                <input class=\"input-xxlarge\" id=\"modal_bb_edit_cup_name\" type='text' value=\""+cup_name+"\" />"+
            "                                            </div>" +
            "                                        </div>" +
            "                                        <div class=\"profile-info-row\">" +
            ...
            "                                            <div class=\"profile-info-value\">" +
            "                                                <select class=\"form-control\" id=\"modal_bb_select_status\" data-placeholder=\"请选择比赛的状态\">" +
            ...
            "                                            <div class=\"profile-info-value\">" +
            "                                                <select class=\"form-control\" id=\"modal_bb_select_level\" data-placeholder=\"请选择比赛的等级\">" +
            ...
            if (e.which == 13){
                if (search_focus){
                    ...
                    if (search_key == ""){
                        ...
                        $("#bb_table_head_title").html("搜索赛事结果列表");
                        $("#yesterday_data").addClass("disabled");
                        $("#tomorrow_date").addClass("disabled");
                        $("#date_picker_bb").attr("disabled",true);
                        $("#more_data_p").addClass("hidden");
    ...
    function searchContest(key,startTime){
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: appName+"/event/basketball/search",
                ...
                if (data.code == 200) {
                    if (data.number > 0){
                        ...
                        $.each(data.contests, function (index, contest) {
                            ...
                            var htmlStr = showBbData(contest.contest_id, contest.color, contest.cup_name,contest.cup_id, contest.h_t.name,contest.home_team, contest.a_t.name,contest.away_team,
                                    contest.start_time, contest.home_scores, contest.away_scores, contest.status, contest.bet_count, contest.settle,
                                    contest.odds_type, contest.lock_flag, false, contest.level, contest.init_count, contest.home_count, contest.away_count);
